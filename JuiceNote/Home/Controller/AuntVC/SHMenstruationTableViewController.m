...
- (void)firstButtonAction:(UIButton *)sender{
    ...
    CYAlertAction *actionCamera = [CYAlertAction actionWithTitle:@"确定" handler:^(UIAlertAction *action) {
        ...
        //设置上次姨妈时间
        account.accountHome.lastAuntDate = dateCell.rightLabel.text;
        
        //姨妈间隔cell
        SHMenstruationTableViewCell *intervalCell = self.tableView.visibleCells[1];
        //设置姨妈间隔时间
        account.accountHome.interval = [intervalCell.rightLabel.text substringToIndex:2];
        //存储到沙盒
...
- (void)dateChanged:(id)sender {
    ...
    //获取账户信息
    XJJAccount *account = [XJJAccountTool account];
    account.accountHome.lastAuntDate = [formatter stringFromDate:dateCell.datePicker.date];
    account.accountHome.interval = [intervalCell.rightLabel.text substringToIndex:2];
    //存储到沙盒
    [XJJAccountTool saveAccount:account];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        if (!account.accountHome.lastAuntDate) {
            NSString *timestamp = [formatter stringFromDate:[NSDate date]];
            cell.rightLabel.text = timestamp;
        } else {
            NSString *timestamp = account.accountHome.lastAuntDate;
            cell.rightLabel.text = timestamp;
        }
        if (indexPath == self.selectedIndexPath) {
            [cell.datePicker addTarget:self action:@selector(dateChanged:) forControlEvents:UIControlEventValueChanged];
            if (account.accountHome.lastAuntDate) {
                cell.datePicker.date = [formatter dateFromString:account.accountHome.lastAuntDate];
            }else{
        ...
        if (!account.accountHome.interval) {
            cell.rightLabel.text = @"28天";
        }else{
            cell.rightLabel.text = account.accountHome.interval;
        }
        if (indexPath == self.selectedIndexPath) {
            cell.datePicker.hidden = YES;
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    ...
    //获取账户信息
    XJJAccount *account = [XJJAccountTool account];
    account.accountHome.interval = [self.dayArray objectAtIndex:row];
    //存储到沙盒
    [XJJAccountTool saveAccount:account];
}
