...
- (void)viewDidLoad {
    ...
    self.label.text = account.nickName;
    ...
    if (imageModel.iconImage) {
        ...
        [icon sd_setImageWithURL:[NSURL URLWithString:account.iconURL] completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, NSURL *imageURL) {
...
//    NSString *timestamp = [obj objectForKey:@"timestamp"];//时间
    ...
    NSString *textString = @"内容";
    NSString *iconURL = @"头像";
    NSString *nickName = @"昵称";
    NSString *timestamp = @"时间";
